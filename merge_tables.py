...
SINGLE_FILE_VARIABLE_COLUMNS = [8,9,10,11,12,13,21,22,23,24] # When dropping evidence column


def concat_cols(row, cols):
    col_values = ["_".join(str(col).split("_")[:2]) for col in cols if row[col] != 0 and not np.isnan(row[col])]
...
def main():
    ...
    name_info = [(path.stem.split('_')[0], path.stem.split('_')[1]) for path in table_paths]
    print(name_info)
    dataframe_dict = {}
    for i in range(len(name_info)):
        dataframe_dict[f"{name_info[i][0]}-{name_info[i][1]}"] = pd.read_excel(table_paths[i], sheet_name=0)


    for key, df in dataframe_dict.items():
        df.drop(df.columns[14], axis=1, inplace=True)
        df_cols = [col for col in df.columns if col.endswith("_log2FC") or col.endswith("_peak_height") or col.endswith("_relative_density")]
        dataframe_dict[key] = df.rename(columns={c: f"{key}_{c}" for c in df.columns if c in df_cols})

    data_frames = [dataframe_dict[key] for key in list(dataframe_dict.keys())]

    new_df = pd.merge(data_frames[0], data_frames[1], on=["Type", "Identifier", "Genome", "Start", "Stop", "Strand", "Locus_tag", "Codon_count", "Start_codon", "Stop_codon", "15nt_window", "Nucleotide_Seq", "Amino_Acid_Seq", "5'-distance", "3'-distance"], how= "outer")
    ...
    for i in range(2,len(data_frames)):
        new_df = pd.merge(new_df, data_frames[i], on=["Type", "Identifier", "Genome", "Start", "Stop", "Strand", "Locus_tag", "Codon_count", "Start_codon", "Stop_codon", "15nt_window", "Nucleotide_Seq", "Amino_Acid_Seq", "5'-distance", "3'-distance"], how= "outer")
        drop_cols = [col for col in new_df.columns if col.endswith("_y") ]
        new_df.drop(drop_cols, axis=1, inplace=True)
        rename_cols = [col for col in new_df.columns if col.endswith("_x") ]
        new_df.rename(columns={c: c[:-2] for c in rename_cols}, inplace=True)
    ...
    relative_density_cols = [col for col in new_df.columns if col.endswith("_relative_density") ]
    rpkm_cols = [col for col in new_df.columns if col.endswith("_rpkm") ]
    TE_cols = [col for col in new_df.columns if col.endswith("_TE") ]
    # new_df[log2FC_cols] = new_df[log2FC_cols].fillna(0)

    new_df["Evidence"] = new_df.apply(lambda row: concat_cols(row, log2FC_cols), axis=1)

    new_header = ["Identifier", "Type", "Genome", "Start", "Stop", "Strand", "Locus_tag", "Codon_count"] + \
                 [col for col in log2FC_cols] + \
                 ["Evidence", "Start_codon", "Stop_codon", "15nt_window", "Nucleotide_Seq", "Amino_Acid_Seq", "5'-distance", "3'-distance"] + \
    ...
    for column in new_df:
        if column not in ["Nucleotide_Seq", "Amino_Acid_Seq", "Evidence"]:
            column_width = max(new_df[column].astype(str).map(len).max(), len(column)) + 2
        else:
