...
class TestCombinedDataDetection:
    ...
    def test_combined_detection_filters_wrong_frame(self):
        """Test that ORFs in wrong frame are filtered"""
        codon_dict_tis = {
            ("chr1:0-4:+", 0): ["ATG", 10]  # Start at 2
        }
        codon_dict_tts = {
            ("chr1:10-14:+", 0): ["TAA", 20]  # Stop at 14
        }
        ...
        # Should check frame compatibility
        # get_frame(start) should equal get_frame(stop - 2)
        # This test needs proper coordinates to work

#     def test_combined_detection_filters_too_long(self):
#         """Test that ORFs exceeding max_length are filtered"""
#         codon_dict_tis = {
#             ("chr1:0-4:+", 0): ["ATG", 10]
#         }
#         codon_dict_tts = {
#             ("chr1:900-904:+", 0): ["TAA", 20]
#         }
...
#             offset_tis=0,
#             offset_tts=0,
#             max_orf_length=100  # ORF would be ~900bp
#         )

#         # Should filter out long ORF
#         # Check result based on actual calculation

#     def test_combined_detection_minus_strand(self):
#         """Test combined detection on minus strand"""
#         codon_dict_tis = {
#             ("chr1:10-14:-", 0): ["ATG", 15]
#         }
#         codon_dict_tts = {
...
#         # Minus strand logic: start > stop
#         assert isinstance(result, dict)

#     def test_combined_detection_no_valid_pairs(self):
#         """Test when no valid start-stop pairs exist"""
#         codon_dict_tis = {
#             ("chr1:100-104:+", 0): ["ATG", 10]  # Start after stop
#         }
#         codon_dict_tts = {
#             ("chr1:10-14:+", 0): ["TAA", 20]
#         }
...
#         # Should find no valid ORFs
#         assert len(result) == 0 or all(len(v) == 0 for v in result.values())

#     def test_combined_detection_missing_chromosome(self):
#         """Test when chromosomes don't match between TIS and TTS"""
#         codon_dict_tis = {
#             ("chr1:2-6:+", 0): ["ATG", 10]
#         }
#         codon_dict_tts = {
#             ("chr2:10-14:+", 0): ["TAA", 20]  # Different chromosome
#         }
...
#     def test_combined_detection_returns_rpkm_values(self):
#         """Test that returned dict contains RPKM values"""
#         codon_dict_tis = {
#             ("chr1:0-4:+", 0): ["ATG", 100]
#         }
#         codon_dict_tts = {
#             ("chr1:30-34:+", 0): ["TAA", 200]
#         }
...
#         # Check structure: {(chrom, strand): {(start, stop): (start_rpm, stop_rpm)}}
#         if len(result) > 0:
#             for chrom_strand, orfs in result.items():
#                 for orf_coords, rpm_values in orfs.items():
#                     assert len(rpm_values) == 2  # (start_rpm, stop_rpm)
#                     assert isinstance(rpm_values[0], (int, float))
#                     assert isinstance(rpm_values[1], (int, float))